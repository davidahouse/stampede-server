...
async function storeBuildConfig(buildPath, config) {
  await client.store("stampede-" + buildPath + "-config", config);
}

async function fetchBuildConfig(buildPath) {
  const config = await client.fetch("stampede-" + buildPath + "-config");
  return config;
}

async function removeBuildConfig(buildPath) {
  await client.remove("stampede-" + buildPath + "-config");
}
...
module.exports.storeBuildConfig = storeBuildConfig;
module.exports.fetchBuildConfig = fetchBuildConfig;
module.exports.removeBuildConfig = removeBuildConfig;
