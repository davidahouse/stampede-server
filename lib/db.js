 ...
 * @param {*} title
 */
async function fetchArtifactMetadata(taskID, title) {
  const query =
    "SELECT metadata from stampede.taskArtifacts WHERE task_id = $1 and title = $2";
  return await execute("fetchArtifactMetadata", query, [taskID, title]);
}
...
async function fetchLatestArtifact(
  ...
  let query =
    "SELECT stampede.taskArtifacts.* \
    from stampede.taskArtifacts, stampede.builds, stampede.tasks where \
    builds.build_id = tasks.build_id AND \
    tasks.task_id = taskArtifacts.task_id AND \
    title = $1";
  ...
  return await execute("fetchLatestArtifact", query, queryParams);
}
...
module.exports.fetchArtifactMetadata = fetchArtifactMetadata;
module.exports.fetchLatestArtifact = fetchLatestArtifact;
