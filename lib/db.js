...
async function execute(operation, sql, params) {
  ...
  if (logSlowQueries === "true" && finished - start > 50.0) {
...
async function updateTables(client) {
  ...
  if (currentVersion === 1) {
    ...
    client.query("ALTER TABLE stampede.builds ADD COLUMN source varchar;");
    client.query("UPDATE stampede.version SET version = 2");
  }
}
