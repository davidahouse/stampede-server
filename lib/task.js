...
async function startTasks(owner, repo, buildKey, sha, tasks, buildPath, buildNumber,
  ...
  if (repoConfig.onCompleted != null) {
    console.log('--- adding onCompleted tasks to pending list')
    for (let index = 0; index < tasks.length; index++) {
      const task = repoConfig.onCompleted[index]
...
async function startTask(owner, repo, buildKey, sha, task, taskNumber, buildPath, buildNumber,
  scm, scmDetails, cache, repoConfig, serverConf) {
    ...
    const taskConfig = await taskDetail.taskConfig(task.id, repoConfig, task, cache)
    const started_at = new Date()
 ...
 * @param {*} serverConf 
 */
async function addTaskToPending(owner, repo, buildKey, parentTaskID, task, taskNumber, buildPath, buildNumber,
  scm, scmDetails, cache, repoConfig, serverConf) {
