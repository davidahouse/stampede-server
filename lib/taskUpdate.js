  ...
  try {
    ...
    if (event.status === "completed") {
      ...
      if (event.result.conclusion === "success") {
        ...
        if (onSuccessTasks != null) {
          for (let index = 0; index < onSuccessTasks.length; index++) {
            await cache.addTaskToActiveList(
              onSuccessTasks[index].buildID,
              onSuccessTasks[index].taskID
            );
          }
          for (let index = 0; index < onSuccessTasks.length; index++) {
            await task.queuePendingTask(
              onSuccessTasks[index],
              scm,
              cache,
              serverConf
            );
        ...
        if (onFailureTasks != null) {
          for (let index = 0; index < onFailureTasks.length; index++) {
            await cache.addTaskToActiveList(
              onFailureTasks[index].buildID,
              onFailureTasks[index].taskID
            );
          }
          for (let index = 0; index < onFailureTasks.length; index++) {
            await task.queuePendingTask(
              onFailureTasks[index],
              scm,
              cache,
              serverConf
            );
      ...
      if (remainingTasks == null || remainingTasks.length === 0) {
        // Check to see if we have any pending onCompleted tasks. These happen at the end
        // of the build.
        // if we have some, the build isn't fully complete yet, so queue these tasks. Otherwise
        // build is totally complete and we can send out notifications and cleanup
        const onCompletedTasks = await cache.pendingTasks(
          event.buildID + "-pending"
        );
