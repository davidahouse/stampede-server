...
async function handle(job, serverConf, cache, scm) {
  try {
    ...
    if (job.buildType === "Pull Request") {
      ...
      buildDetails.sha = job.scmConfig.headSha;
      buildDetails.pullRequest = {
        number: job.scmConfig.pullRequest.number,
        title: job.scmConfig.pullRequest.title,
        head: {
          ref: job.scmConfig.pullRequest.headRef,
          sha: job.scmConfig.pullRequest.headSha
        },
        base: {
          ref: job.scmConfig.pullRequest.baseRef,
          sha: job.scmConfig.pullRequest.baseSha
        }
      };
